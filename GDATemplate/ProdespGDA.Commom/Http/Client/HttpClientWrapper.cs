...
namespace ProdespGDA.Commom.Http.Client
{
    public sealed class HttpClientWrapper : IHttpClient
    {
        ...
        public CookieCollection Cookies(Uri uri) => new CookieCollection() { cookieContainer.GetCookies(uri) };
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            //string Body = responseMessage.Content.ReadAsStringAsync().Result; //.ConfigureAwait(false);
        ...
        private async Task<HttpResponseMessage> HttpResponseMessage(HttpRequest request, CancellationToken cancellationToken)
        {
            ...
            if (request.HttpMethod.Equals(HttpMethod.Delete) || request.HttpMethod.Equals(HttpMethod.Post) || request.HttpMethod.Equals(HttpMethod.Put) || request.HttpMethod.Equals(new HttpMethod("PATCH")))
            {
                ...
                if (request.Body != null)
                {
                    ...
                    "text/xml;charset=\"utf-8\"", StringComparison.OrdinalIgnoreCase))
                    {
                        requestMessage.Content = new StringContent((string)request.Body, Encoding.UTF8, "text/xml");
                        //                        requestMessage.Headers.Clear();
                        defaultHttpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("text/xml"));
                        requestMessage.Content = new StringContent((string)request.Body, Encoding.UTF8, "text/xml");
                        requestMessage.Content.Headers.ContentType = new MediaTypeHeaderValue("text/xml");
                        HttpResponseMessage response = await defaultHttpClient.SendAsync(requestMessage);
                        return response;
                    }
                    else if (request.Headers.ContainsKey("content-type") && string.Equals(request.Headers["content-type"],
                        "application/soap+xml; charset=utf-8", StringComparison.OrdinalIgnoreCase))
                    {
                        requestMessage.Content = new StringContent((string)request.Body ?? string.Empty, Encoding.UTF8,
                            "application/xml");
                    }
            ...
            return await defaultHttpClient.SendAsync(requestMessage, cancellationToken).ConfigureAwait(false);
        }
